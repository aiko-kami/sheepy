...
function useFileUpload(options) {
	...
	const uploadFile = async (file) => {
		...
		try {
			...
			if (!abortController.signal.aborted) {
				setFileItems((prev) => prev.map((item) => (item.id === fileId ? { ...item, status: "error", progress: 0 } : item)));
...
const CloudUploadIcon = () => (
	<svg width="24" height="24" viewBox="0 0 24 24" className="tiptap-image-upload-icon" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
		<path
...
const FileIcon = () => (
	<svg width="43" height="57" viewBox="0 0 43 57" fill="currentColor" className="tiptap-image-upload-dropzone-rect-primary" xmlns="http://www.w3.org/2000/svg">
		<path
...
const FileCornerIcon = () => (
	<svg width="10" height="10" className="tiptap-image-upload-dropzone-rect-secondary" viewBox="0 0 10 10" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
...
const ImageUploadPreview = ({ fileItem, onRemove }) => {
	...
	return (
		<div className="tiptap-image-upload-preview">
			{fileItem.status === "uploading" && <div className="tiptap-image-upload-progress" style={{ width: `${fileItem.progress}%` }} />}
			<div className="tiptap-image-upload-preview-content">
				...
				<div className="tiptap-image-upload-actions">
					{fileItem.status === "uploading" && <span className="tiptap-image-upload-progress-text">{fileItem.progress}%</span>}
					<Button
						type="button"
						data-style="ghost"
						onClick={(e) => {
			...
			{hasFiles && (
				<div className="tiptap-image-upload-previews">
					...
					{fileItems.map((fileItem) => (
						<ImageUploadPreview key={fileItem.id} fileItem={fileItem} onRemove={() => removeFileItem(fileItem.id)} />
					))}
				</div>
			)}
			<input ref={inputRef} name="file" accept={accept} type="file" multiple={limit > 1} onChange={handleChange} onClick={(e) => e.stopPropagation()} />
		</NodeViewWrapper>
	);
};
