...
import { ApiValidateEmail } from "@/lib/api/auth";
import { Button } from "@/components/Buttons/Buttons";
import Triforce from "@/components/Loaders/Triforce";

import { useRouter } from "next/navigation";

const VerifyEmailMenu = ({ emailValidationId }) => {
	...
	useEffect(() => {
		...
		const validateEmail = async () => {
			try {
				const response = await ApiValidateEmail(emailValidationId);
				setStatus("success");
				setMessage("Your email has been successfully verified. Welcome to the community!");
			} catch (error) {
				setStatus("error");
				setMessage(error.message || "An unexpected error occurred during the email verification.");
			}
		};
