				...
				{objectives.map((objective, index) => (
					...
					>
						...
						{!disabled && (
							<button
								type="button"
								className="ml-1.5 text-gray-600 hover:text-gray-800 transition duration-150 ease-in-out"
								onClick={() => removeObjective(objective)}
								aria-label={`Remove ${objective}`}
							>
								<IoCloseCircleOutline className="text-lg mt-0.5" title="Remove objective" />
							</button>
						)}
					</motion.div>
				))}
